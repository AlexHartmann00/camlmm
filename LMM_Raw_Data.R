data <- read.csv("project_data/full_dataset.csv")

data <- data[data$comorbid != "True",]
data <- data[data$gender != "",]

data <- data[, c("user_id", "X19.29", "X30.39", "X..18", "X..40", "comorbid", "gender", "group_anxiety", "group_depression", "day_of_week", "created_at", "VADER_NEG")]
...
data$anxiety <- ifelse(data$group_anxiety == "True",1,0)
...
#x <- optimize_model_fit(data, c(2,3,4), c(2,3))
...
model <- lmer(
  VADER_NEG ~ gender_female +
    ...
    anxiety +
    weekend + (1|user_id) +
    bs(hour,knots = c(6,12, 18),degree = 2) + X19.29 + X30.39 + X..18 + X..40
  ,data
)
...
write.csv(data_, "project_data/residuals_lmm_vader_raw.csv")
